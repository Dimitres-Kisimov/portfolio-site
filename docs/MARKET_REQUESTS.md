...
| Prescriptive "what should we do" (not just describe) | `revops-optimizer`, `distributor-intelligence-platform` (optimizers → one € plan + action cards; `revops-optimizer` now also gates every recommended price move accept/hold on the same Monte-Carlo draws — 17 accept / 12 hold, labelled a screening discipline, not a guarantee) |
...
| Document understanding / IDP with validation + confidence gate | `doc-extract-agent` (6-stage pipeline, totals cross-check, human routing; the combined confidence + business-rule gate lifts measured auto-post precision 70% → 87.5%, and an extraction-error cost model prices every gating policy — auto-post pays only above 98.4% precision; the validation layer is worth ≈€109,333/yr in the model) |
